...
% the one where it plots WRF4 to NAM and GFS
%% setup
clear; clc; close all;
...
tic
dfile = 'wrf4_rtg_20150803_20150804.nc';
...
dfilec = 'wrf4_cold_20150803_20150804.nc';
...
%% Plotting
hold on;
plot(time_nam,ws_nam,'linewidth',1,'color',[0.4940, 0.1840, 0.5560]);
plot(time_gfs,ws_gfs,'linewidth',1,'color',[0.4660 0.6740 0.1880]);
plot(wrf_dtimec(ind),wrf_wsc(ind),'color',[0, 0.4470, 0.7410],'linewidth',2); 
plot(wrf_dtime(ind),wrf_ws(ind),'color',[0.6350, 0.0780, 0.1840],'linewidth',2);
plot(wrf_dtime(ind),ndbc_ws2(ind),'k-','linewidth',3);

title(sprintf('Wind Speeds at %s WRF 4.0',buoy));
...
fname = sprintf('output/WRF4_RTG_COLD/buoy%s_WRF4RC_nam_gfs_hist_comparison_%s_%s',buoy,datestr(wrf_dtime(ind(1)),29),datestr(wrf_dtime(ind(end)),29));
...
outfile = sprintf('output/WRF4_RTG_COLD/metrics_WRF4RC_all_%s_%s_%s.csv',buoy,datestr(wrf_dtime(ind(1)),29),datestr(wrf_dtime(ind(end)),29));
fid = fopen(outfile,'w');
